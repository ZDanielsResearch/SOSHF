function [index,thresh] = loss_optimize_sf_c(features,labels,minLeaf,imbalance)
...
% if sum(isnan(encoderWeights)) > 0 || sum(isinf(encoderWeights))
%     keyboard;
% end
...
for i = 1:1:numFeatures
    ...
    for j = 2:1:numFeatureVals
        ...
        val = mi(double(features(:,i) >= thresh),initial);
        if val >= bestVal
            bestIndex = i;
            bestThresh = thresh;
            bestVal = val;
