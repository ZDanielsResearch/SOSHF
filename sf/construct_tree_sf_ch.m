function tree = construct_tree_sf_ch(features,labels,minLeaf,labelSampling,featureSampling,instanceSampling,usedWeightParam)
...
maxNode = 2;
weights = zeros(numInstances*10,2);
...
while sum(currentNode > 0) ~= 0
    ...
    for i = 1:1:length(nodeIDs)
        ...
        [index, thresh] = loss_optimize_sf_ch(featuresNode2,labelsNode,minLeaf,imbalance);
        
        assignments = featuresNode(:,index) >= thresh;
...
%             end
        ...
        weights(nodeIDs(i),:) = [index,thresh];
    end
end
...
oobPredictions = predict_tree_sf(tree,featuresOld);
